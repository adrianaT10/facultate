...
for img_folder in image_folders_ckp:
    ...
    image_paths_ckp.append((images[0], 0))
    all_image_paths.append((images[0], 0))
...
with in_db.begin(write=True) as in_txn:
    ...
    # # images from CKP
    # for (img_path, label) in train_data_ckp:
...
with in_db.begin(write=True) as in_txn:
    ...
    # # images from CKP
    # for (img_path, label) in test_data_ckp:
    #     img = detect_face(img_path)
    #     if img == None:
    #         print "face not detected"
    #         continue
...
print '\nGenerating mean..'
os.system('bash generate_mean.sh')
