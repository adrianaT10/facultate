...
class Layer:
    ...
    def forward(self, inputs):
        # print inputs
        # print inputs.shape
        # print self.inputs_no
        assert(inputs.shape == (self.inputs_no, 1))

        # TODO (2.a)
        # -> compute self.a and self.outputs
        self.a = np.tensordot(self.weights, inputs, 1) + self.biases
    ...
    def backward(self, inputs, output_errors):
        ...
        self.g_weights = output_errors * np.transpose(inputs)

        # TODO (2.b.iii)
        # Compute and return the gradients w.r.t the inputs of this layer
        aux = np.tensordot(np.transpose(output_errors), self.weights, 1)
        
        g = np.transpose(aux) * self.f(inputs, True)
