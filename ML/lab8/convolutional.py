...
class ConvolutionalLayer(LayerInterface):
            ...
            for i in range(self.outputs_height):
                for j in range(self.outputs_width):
                    ...
                    for m in range(self.inputs_depth):
                        teta = self.weights[n][m]
                        x = inputs[m, i*self.stride : i*self.stride+self.k, j*self.stride : j*self.stride+self.k]

                        summ += np.sum(np.multiply(teta, x))
                    self.outputs[n][i][j] = summ + self.biases[n]
    ...
    def backward(self, inputs, output_errors):
        ...
        for n in range(self.outputs_depth):
            self.g_biases[n] = np.sum(output_errors[n, :, :])
            ...
            for m in range(self.inputs_depth):
                for p in range(self.k):
                    for q in range(self.k):
                        x = inputs[m, p : self.outputs_height*self.stride+p, q:self.outputs_width*self.stride+q]
                        self.g_weights[n][m][p][q] = np.sum(np.multiply(x, output_errors[n]))                        
        ...
        for m in range(self.inputs_depth):
            for i in range(self.inputs_height):
                for j in range(self.inputs_width):
                    summ = 0.0
                    for n in range(self.outputs_depth):
                        for p in range(self.k):
                            for q in range(self.k):
                                iindex = (i-p)/self.stride
                                jindex = (j-q)/self.stride
                                if iindex < self.outputs_height and jindex < self.outputs_width \
                                    and iindex >= 0 and jindex >= 0:
                                    summ += self.weights[n][m][p][q] * output_errors[n][iindex][jindex]
                    delta[m][i][j] = summ
